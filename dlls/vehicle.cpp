...
void CFuncVehicle::Blocked( CBaseEntity *pOther )
{
	...
	float minx = V_min( vBackLeft.x, vBackRight.x );
	float miny = V_min( vBackLeft.y, vBackRight.y );
	float maxx = V_max( vBackLeft.x, vBackRight.x );
	float maxy = V_max( vBackLeft.y, vBackRight.y );
