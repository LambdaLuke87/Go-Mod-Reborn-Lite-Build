...
Schedule_t slCleanFollow[] =
	{
		{tlCleanFollow,
			ARRAYSIZE(tlCleanFollow),
			bits_COND_NEW_ENEMY |
...
Schedule_t slCleanFollowScared[] =
	{
		{tlCleanFollowScared,
			ARRAYSIZE(tlCleanFollowScared),
			bits_COND_NEW_ENEMY |
				bits_COND_HEAR_SOUND |
				bits_COND_LIGHT_DAMAGE |
				bits_COND_HEAVY_DAMAGE,
...
Task_t tlCleanFaceTargetScared[] =
	{
...
Schedule_t slCleanFaceTargetScared[] =
	{
		{tlCleanFaceTargetScared,
			ARRAYSIZE(tlCleanFaceTargetScared),
			bits_COND_HEAR_SOUND |
				bits_COND_NEW_ENEMY,
			bits_SOUND_DANGER,
			"FaceTargetScared"},
...
Task_t tlCleanStopFollowing[] =
	{
		{TASK_CANT_FOLLOW, (float)0},
};

Schedule_t slCleanStopFollowing[] =
	{
		{tlCleanStopFollowing,
			ARRAYSIZE(tlCleanStopFollowing),
			0,
			0,
			"StopFollowing"},
};
...
Schedule_t slCleanHeal[] =
	{
		{tlCleanHeal,
			ARRAYSIZE(tlCleanHeal),
...
Schedule_t slCleanFaceTarget[] =
	{
		{tlCleanFaceTarget,
			ARRAYSIZE(tlCleanFaceTarget),
...
Schedule_t slCleanSciPanic[] =
	{
		{tlCleanSciPanic,
			ARRAYSIZE(tlCleanSciPanic),
			0,
			0,
			"SciPanic"},
};
...
Schedule_t slCleanIdleSciStand[] =
	{
		{tlCleanIdleSciStand,
			ARRAYSIZE(tlCleanIdleSciStand),
			bits_COND_NEW_ENEMY |
				bits_COND_LIGHT_DAMAGE |
				bits_COND_HEAVY_DAMAGE |
				bits_COND_HEAR_SOUND |
...
Task_t tlCleansuitScientistCover[] =
	{
...
Schedule_t slCleansuitScientistCover[] =
	{
		{tlCleansuitScientistCover,
			ARRAYSIZE(tlCleansuitScientistCover),
			bits_COND_NEW_ENEMY,
			0,
			"ScientistCover"},
};
...
Task_t tlCleansuitScientistHide[] =
	{
		{TASK_SET_FAIL_SCHEDULE, (float)SCHED_PANIC}, // If you fail, just panic!
		{TASK_STOP_MOVING, (float)0},
...
Schedule_t slCleansuitScientistHide[] =
	{
		{tlCleansuitScientistHide,
			ARRAYSIZE(tlCleansuitScientistHide),
			bits_COND_NEW_ENEMY |
				bits_COND_HEAR_SOUND |
				bits_COND_SEE_ENEMY |
				bits_COND_SEE_HATE |
...
Task_t tlCleansuitScientistStartle[] =
	{
		{TASK_SET_FAIL_SCHEDULE, (float)SCHED_PANIC}, // If you fail, just panic!
...
Schedule_t slCleansuitScientistStartle[] =
	{
		{tlCleansuitScientistStartle,
			ARRAYSIZE(tlCleansuitScientistStartle),
			bits_COND_NEW_ENEMY |
				bits_COND_SEE_ENEMY |
				bits_COND_SEE_HATE |
				bits_COND_SEE_FEAR |
...
DEFINE_CUSTOM_SCHEDULES(CCleansuitScientist){
	...
	slCleanIdleSciStand,
	slCleanFear,
	slCleansuitScientistCover,
	slCleansuitScientistHide,
	slCleansuitScientistStartle,
	slCleanHeal,
	slCleanStopFollowing,
	slCleanSciPanic,
	slCleanFollowScared,
	slCleanFaceTargetScared,
};
...
bool CCleansuitScientist::TakeDamage(entvars_t* pevInflictor, entvars_t* pevAttacker, float flDamage, int bitsDamageType)
{
...
Schedule_t* CCleansuitScientist::GetScheduleOfType(int Type)
{
	...
	case SCHED_TARGET_FACE:
		...
		if (psched == slIdleStand)
			return slCleanFaceTarget; // override this for different target face behavior
		else
			return psched;

	case SCHED_TARGET_CHASE:
		return slCleanFollow;

	case SCHED_CANT_FOLLOW:
		return slCleanStopFollowing;

	case SCHED_PANIC:
		return slCleanSciPanic;

	case SCHED_TARGET_CHASE_SCARED:
		return slCleanFollowScared;

	case SCHED_TARGET_FACE_SCARED:
		return slCleanFaceTargetScared;
	...
	case SCHED_IDLE_STAND:
		...
		if (psched == slIdleStand)
			return slCleanIdleSciStand;
		else
			return psched;

	case SCHED_HIDE:
		return slCleansuitScientistHide;
...
//=========================================================
bool CSittingCleansuitScientist::FIdleSpeak()
{
