...
* 
* 
* Originally made by JujU from HL: Invasion team, all credit goes to them
* 
* 
*/
...
LINK_ENTITY_TO_CLASS(monster_charger, CFlybee);
...
void CFlybee::Spawn()
{
	...
	pev->health = gSkillData.chargerHealth;
...
void CFlybee::HandleAnimEvent(MonsterEvent_t* pEvent)
{
	...
	case FLYBEE_AE_HIT:
	{
		...
		if (tr.pHit)
		{
			...
			pEntity->TakeDamage(pev, pev, gSkillData.chargerDmgMelee, DMG_CLUB);
		}
		break;
	}
	...
	case FLYBEE_AE_ATTACK2:
	{
		...
		RadiusDamage(vecEnd, pev, pev, gSkillData.chargerDmgProjectile, CLASS_ALIEN_MONSTER, DMG_SHOCK);
