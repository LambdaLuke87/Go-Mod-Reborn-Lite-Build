...
{
	SENTENCEG_PlayRndSz(ENT(pev), "BC_IDLE", 1, ATTN_NORM, 0, PITCH_NORM);
}
...
void CBullsquid::PainSound()
{
	...
	SENTENCEG_PlayRndSz(ENT(pev), "BC_PAIN", 1, ATTN_NORM, 0, iPitch);
}
...
{
	SENTENCEG_PlayRndSz(ENT(pev), "BC_DEATH", 1, ATTN_NORM, 0, PITCH_NORM);
}
...
{
	SENTENCEG_PlayRndSz(ENT(pev), "BC_ATTACKNORMAL", 1, ATTN_NORM, 0, PITCH_NORM);
}
...
void CBullsquid::StartTask(Task_t* pTask)
{
	...
	{
		SENTENCEG_PlayRndSz(ENT(pev), "BC_ATTACKGROWL", 1, ATTN_NORM, 0, PITCH_NORM);
