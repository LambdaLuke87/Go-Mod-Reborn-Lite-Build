...
TYPEDESCRIPTION CBasePlayer::m_playerSaveData[] =
	{
		...
		DEFINE_FIELD(CBasePlayer, m_hManipulateNPC, FIELD_EHANDLE),
		...
		DEFINE_FIELD(CBasePlayer, m_fToolManipulatorMode, FIELD_BOOLEAN),
...
void CBasePlayer::DefaultDeathSound()
{
	// water death sounds
	/*
	if (pev->waterlevel == 3)
	...
	if (m_fInitHUD)
	{
		...
		if (g_pGameRules->FAllowFlashlight())
		{
			if (HasSuit())
			{
				...
				{
					MESSAGE_BEGIN(MSG_ONE, gmsgFlashlight, NULL, pev);
					WRITE_BYTE(static_cast<int>(FlashlightIsOn()));
					WRITE_BYTE(m_iFlashBattery);
					MESSAGE_END();
				}
			}
...
bool CBasePlayer::Menu_Char_Input(int inp)
{
	...
	const auto killAndGib = OBS_NONE == pev->iuser1 && m_iTeamNum != m_iNewTeamNum;
