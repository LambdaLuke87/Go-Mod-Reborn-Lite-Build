...
#define BIG_CHILDCLASS_ARTIFICIAL "monster_babycrab_artificial"
#define BIG_CHILDCLASS_ALLIED "monster_babycrab_allied"
...
void CBigMomma::LayHeadcrab()
{
	...
	if (m_AltClass) // fix maternal battles
		babycrab = BIG_CHILDCLASS_ALLIED;
	else
	{
		// include children as an entity generated by the menu
		if (m_MenuCreated)
			babycrab = BIG_CHILDCLASS_ARTIFICIAL;
		else
			babycrab = BIG_CHILDCLASS;
	}

	CBaseEntity* pChild = CBaseEntity::Create(babycrab, pev->origin, pev->angles, edict());
