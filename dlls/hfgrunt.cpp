...
#define HFGRUNT_SENTENCE_VOLUME (float)0.35			// volume of grunt sentences
...
TYPEDESCRIPTION CHFGrunt::m_SaveData[] =
	{
		...
		//	DEFINE_FIELD( CHFGrunt, m_flLastEnemySightTime, FIELD_TIME ), // don't save, go to zero
		...
		DEFINE_FIELD(CHFGrunt, m_voicePitch, FIELD_INTEGER),
		//  DEFINE_FIELD( CShotgun, m_iBrassShell, FIELD_INTEGER ),
		//  DEFINE_FIELD( CShotgun, m_iShotgunShell, FIELD_INTEGER ),
		DEFINE_FIELD(CHFGrunt, m_iSentence, FIELD_INTEGER),
};

IMPLEMENT_SAVERESTORE(CHFGrunt, CSquadMonster);
...
enum
{
	...
	HFGRUNT_SENT_GREN = 0,
	HFGRUNT_SENT_ALERT,
	HFGRUNT_SENT_MONSTER,
	HFGRUNT_SENT_COVER,
	HFGRUNT_SENT_THROW,
	HFGRUNT_SENT_CHARGE,
	HFGRUNT_SENT_TAUNT,
} HFGRUNT_SENTENCE_TYPES;
...
void CHFGrunt::SpeakSentence()
{
	...
	if (FOkToSpeak())
	{
		SENTENCEG_PlayRndSz(ENT(pev), pGruntSentences[m_iSentence], HFGRUNT_SENTENCE_VOLUME, GRUNT_ATTN, 0, m_voicePitch);
		JustSpoke();
	}
}
...
void CHFGrunt::IdleSound()
{
	...
	{
		if (0 == g_fFGruntQuestion)
		{
			// ask question or make statement
			switch (RANDOM_LONG(0, 2))
			{
			case 0: // check in
				SENTENCEG_PlayRndSz(ENT(pev), "HG_CHECK", HFGRUNT_SENTENCE_VOLUME, ATTN_NORM, 0, m_voicePitch);
				g_fFGruntQuestion = 1;
				break;
			case 1: // question
				SENTENCEG_PlayRndSz(ENT(pev), "HG_QUEST", HFGRUNT_SENTENCE_VOLUME, ATTN_NORM, 0, m_voicePitch);
				g_fFGruntQuestion = 2;
				break;
			case 2: // statement
				SENTENCEG_PlayRndSz(ENT(pev), "HG_IDLE", HFGRUNT_SENTENCE_VOLUME, ATTN_NORM, 0, m_voicePitch);
				break;
			}
		}
		else
		{
			switch (g_fFGruntQuestion)
			{
			case 1: // check in
				SENTENCEG_PlayRndSz(ENT(pev), "HG_CLEAR", HFGRUNT_SENTENCE_VOLUME, ATTN_NORM, 0, m_voicePitch);
				break;
			case 2: // question
				SENTENCEG_PlayRndSz(ENT(pev), "HG_ANSWER", HFGRUNT_SENTENCE_VOLUME, ATTN_NORM, 0, m_voicePitch);
				break;
			}
			g_fFGruntQuestion = 0;
		}
...
void CHFGrunt::PainSound()
{
	if (gpGlobals->time > m_flNextPainTime)
	{
		...
		if ( RANDOM_LONG(0,99) < 5 )
		{
			...
			if (FOkToSpeak())
			{
				SENTENCEG_PlayRndSz(ENT(pev), "HG_PAIN", HFGRUNT_SENTENCE_VOLUME, ATTN_NORM, 0, PITCH_NORM);
				JustSpoke();
				return;
			}
		}
...
IMPLEMENT_CUSTOM_SCHEDULES(CHFGrunt, CSquadMonster);
	...
	if (HasConditions(bits_COND_HEAR_SOUND))
	{
		...
		if (pSound)
		{
			if ((pSound->m_iType & bits_SOUND_DANGER) != 0)
			{
				...
				if (FOkToSpeak())
				{
					SENTENCEG_PlayRndSz(ENT(pev), "HG_GREN", HFGRUNT_SENTENCE_VOLUME, GRUNT_ATTN, 0, m_voicePitch);
					JustSpoke();
				}
	...
	case MONSTERSTATE_COMBAT:
	{
				...
				else
				{
					...
					if (FOkToSpeak()) // && RANDOM_LONG(0,1))
					{
						if ((m_hEnemy != NULL) && m_hEnemy->IsPlayer())
							...
							// monster
							SENTENCEG_PlayRndSz(ENT(pev), "HG_MONST", HFGRUNT_SENTENCE_VOLUME, GRUNT_ATTN, 0, m_voicePitch);
		...
		else if (HasConditions(bits_COND_LIGHT_DAMAGE))
		{
			...
			if (iPercent <= 90 && m_hEnemy != NULL)
			{
				...
				if (FOkToSpeak()) // && RANDOM_LONG(0,1))
				{
					//SENTENCEG_PlayRndSz( ENT(pev), "HG_COVER", HFGRUNT_SENTENCE_VOLUME, GRUNT_ATTN, 0, m_voicePitch);
					m_iSentence = HFGRUNT_SENT_COVER;
					//JustSpoke();
				}
				return GetScheduleOfType(SCHED_TAKE_COVER_FROM_ENEMY);
			}
		...
		else if (HasConditions(bits_COND_ENEMY_OCCLUDED))
		{
			if (HasConditions(bits_COND_CAN_RANGE_ATTACK2) && OccupySlot(bits_SLOTS_HGRUNT_GRENADE))
			{
				...
				if (FOkToSpeak())
				{
					SENTENCEG_PlayRndSz(ENT(pev), "HG_THROW", HFGRUNT_SENTENCE_VOLUME, GRUNT_ATTN, 0, m_voicePitch);
					JustSpoke();
				}
				return GetScheduleOfType(SCHED_RANGE_ATTACK2);
			}
			else if (OccupySlot(bits_SLOTS_HGRUNT_ENGAGE))
			{
				...
				if (FOkToSpeak()) // && RANDOM_LONG(0,1))
				{
					//SENTENCEG_PlayRndSz( ENT(pev), "HG_CHARGE", HFGRUNT_SENTENCE_VOLUME, GRUNT_ATTN, 0, m_voicePitch);
					m_iSentence = HFGRUNT_SENT_CHARGE;
					//JustSpoke();
				}
			...
			else
			{
				...
				if (FOkToSpeak() && RANDOM_LONG(0, 1))
				{
					SENTENCEG_PlayRndSz(ENT(pev), "HG_TAUNT", HFGRUNT_SENTENCE_VOLUME, GRUNT_ATTN, 0, m_voicePitch);
					JustSpoke();
				}
	...
	case SCHED_TAKE_COVER_FROM_ENEMY:
	{
		if (InSquad())
		{
			if (g_iSkillLevel == SKILL_HARD && HasConditions(bits_COND_CAN_RANGE_ATTACK2) && OccupySlot(bits_SLOTS_HGRUNT_GRENADE))
			{
				...
				return slFGruntTossGrenadeCover;
			}
			else
			{
...
//=========================================================
// CHFGruntRepel - when triggered, spawns a monster_human_friendly_grunt
...
char* CDeadHFGrunt::m_szPoses[] = {"deadstomach", "deadside", "deadsitting"};

bool CDeadHFGrunt::KeyValue(KeyValueData* pkvd)
{
...
LINK_ENTITY_TO_CLASS(monster_fhgrunt_dead, CDeadHFGrunt);
